...
export const zh: Translations = {
  ...
  settings: {
    ...
    appearance: {
      ...
      noThemesMatch: query => `没有已安装的主题匹配“${query.trim()}”。`,
      themeDescriptions: {
        github: 'GitHub 浅色默认和深色默认',
        nous: 'GitHub 界面，Nous 蓝色强调色',
        catppuccin: '柔和的粉彩色 — Latte 和 Mocha',
        everforest: '温暖、低对比度的森林绿',
        solarized: '固定对比度的浅色和深色',
        midnight: '深蓝紫色与冷色强调',
        ember: '温暖的深红与青铜 — 锻造氛围',
        mono: '简洁灰度 — 极简且专注',
        cyberpunk: '黑底霓虹绿 — 矩阵终端',
        slate: '冷色板岩蓝 — 专注的开发者主题'
      },
    ...
    customEndpoints: {
      ...
      saving: '保存中…',
      newEndpoint: '新建端点',
      reachableWithModels: count => `端点可访问。找到 ${count} 个模型。`,
      reachable: '端点可访问。',
    ...
    computerUse: {
      loading: '正在检查 Computer Use 状态…',
      unsupported: platform => `此平台不支持 Computer Use（${platform}）。`,
      installBackend: '请在下方安装 cua-driver 后控制此计算机。',
      grantHere: ' 然后在此处授予辅助功能和屏幕录制权限。',
      linuxNote: '通过 X11/XWayland 辅助功能栈控制桌面 — 不会弹出权限提示。',
      windowsNote: '首次运行可能会显示 Windows SmartScreen 对 cua-driver UIAccess worker 的提示 — 请允许。',
      driverIdentity: '权限附加到 CuaDriver 自身的身份（com.trycua.driver），而不是 Hermes。',
      recheck: '重新检查',
      accessibility: '辅助功能',
      accessibilityHint: '允许 cua-driver 发送点击和按键，并读取辅助功能树。',
      screenRecording: '屏幕录制',
      screenRecordingHint: '允许 cua-driver 捕获应用窗口的屏幕截图。',
      granted: '已授予',
      notGranted: '未授予',
      unknown: '未知',
      driverHealth: '驱动健康状态',
      ready: '就绪',
      notReady: '未就绪',
      computerUseReady: 'Computer Use 已就绪。请让代理捕获应用并进行点击。',
      approveInSystemSettings: '在系统设置中批准',
      permissionDialogMessage: 'macOS 将显示归属于 CuaDriver 的权限对话框。批准后返回此处。',
    ...
    toolsets: {
      ...
      computerUseProviderDescription: '通过 cua-driver 在后台控制计算机 — 不会抢占你的鼠标或焦点。适用于任何模型。',
  ...
  skills: {
    ...
    noDescription: '暂无描述。',
    computerUseDescription: '通过 cua-driver 在后台控制计算机 — 不会抢占你的鼠标或焦点。适用于任何模型。',
    configured: '已配置',
    needsKeys: '需要密钥',
  ...
  ui: {
    ...
    loadingMemoryProviderSettings: '正在加载记忆提供商设置…',
    memoryProviderSettingsFailed: message => `加载记忆提供商设置失败：${message}`,
    retry: '重试',
    dangerZone: '危险区域',
    checkingInstalled: '正在检查已安装内容…',
    confirmUninstall: '确认卸载',
    thisRemoves: consequence => `这将移除${consequence}。此操作无法撤销。`,
    yesUninstall: '是，卸载',
    uninstalling: '正在卸载…',
    uninstallHermes: '卸载 Hermes',
    uninstallDescription: '选择要移除的内容。应用会关闭以完成操作；你可以随时重新打开安装程序。',
    configSettings: label => `${label} 设置`,
    fieldSet: label => `${label} 已设置`,
    fieldNotSet: label => `${label} 未设置`,
    fullConfig: '完整配置…'
  }
}
