      ...
      <Pill tone={tone(granted)}>
        ...
        {granted === true
          ...
          : granted === false
            ...
            : t.settings.computerUse.unknown}
      </Pill>
    </div>
  )
}
...
export function ComputerUsePanel({ onConfiguredChange }: ComputerUsePanelProps) {
  const { t } = useI18n()
  const computerUseCopy = t.settings.computerUse
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      notifyError(err, computerUseCopy.loading)
    } finally {
      setLoading(false)
    }
  }, [computerUseCopy])
  ...
  const grant = useCallback(async () => {
    ...
    try {
      ...
      if (!started.ok) {
        notifyError(new Error('spawn failed'), computerUseCopy.requestPermissionsFailed)
      ...
      notify({
        kind: 'info',
        title: computerUseCopy.approveInSystemSettings,
        message: computerUseCopy.permissionDialogMessage
      })
      ...
      if (activeRef.current) {
        notifyError(err, computerUseCopy.requestPermissionsFailed)
      }
    } finally {
      if (activeRef.current) {
        setGranting(false)
  ...
  }, [computerUseCopy, onConfiguredChange, refresh])
  ...
  if (!status.platform_supported) {
    return <p className="px-1 text-xs text-muted-foreground">{t.settings.computerUse.unsupported(status.platform)}</p>
  }
  ...
  if (!status.installed) {
    return (
      <p className="px-1 text-xs text-muted-foreground">
        ...
        {status.can_grant && t.settings.computerUse.grantHere}
      </p>
    )
  }
  ...
  return (
    <div className="grid gap-2">
      <div className="flex flex-wrap items-center justify-between gap-2 px-1">
        <div className="min-w-0">
          {status.can_grant ? (
            <p className="text-[0.72rem] text-muted-foreground">{t.settings.computerUse.driverIdentity}</p>
          ) : (
            <p className="text-[0.72rem] text-muted-foreground">
              {status.platform === 'linux'
                ? t.settings.computerUse.linuxNote
                : status.platform === 'win32'
                  ? t.settings.computerUse.windowsNote
                  : ''}
            </p>
          )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-background/55 p-2.5">
          ...
          <Pill tone={tone(status.ready)}>
            ...
            {status.ready === true
              ...
              : status.ready === false
                ? t.settings.computerUse.notReady
                : t.settings.computerUse.unknown}
          </Pill>
        </div>
      )}
      ...
      {status.ready ? (
        <div className="flex items-center gap-1.5 px-1 text-xs text-muted-foreground">
          ...
          {t.settings.computerUse.computerUseReady}
        </div>
      ) : (
        status.can_grant && (
          <Button disabled={granting} onClick={() => void grant()} size="sm">
            ...
            {granting ? t.settings.computerUse.waitingForApproval : t.settings.computerUse.grantPermissions}
          </Button>
        )
      )}
    </div>
