...
export function CustomEndpointsSettings({ onConfigSaved, onMainModelChanged }: CustomEndpointsSettingsProps) {
  const { t } = useI18n()
  const loadFailedMessage = t.settings.customEndpoints.loadFailed
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        notifyError(err, loadFailedMessage)
      } finally {
        if (!cancelled) {
          setLoading(false)
        }
  ...
  async function handleSave() {
    try {
      ...
      notifyError(err, t.settings.customEndpoints.saveFailed)
    } finally {
      setSaving(false)
    }
  }
      ...
      if (response.ok) {
        ...
        notify({
          ...
          message: response.models.length
            ? t.settings.customEndpoints.reachableWithModels(response.models.length)
            : t.settings.customEndpoints.reachable
        })
      } else {
        notify({
          kind: response.reachable ? 'warning' : 'error',
          message: response.message || t.settings.customEndpoints.validationFailed
        })
      }
    } catch (err) {
      notifyError(err, t.settings.customEndpoints.validationFailed)
    } finally {
      setTesting(false)
    }
  }
  ...
  async function handleActivate(endpoint: CustomEndpoint) {
    try {
      ...
      notifyError(err, t.settings.customEndpoints.activationFailed)
    } finally {
      setActivating(null)
    }
  }
  ...
  async function handleDelete(endpoint: CustomEndpoint) {
    // The endpoint name is user data; only the confirmation shell is translated.
    if (!(await confirm({ destructive: true, title: t.settings.customEndpoints.deleteTitle(endpoint.name) }))) {
      return
    }
    ...
    try {
      ...
      notifyError(err, t.settings.customEndpoints.deleteFailed)
    } finally {
      setDeleting(null)
    }
  }
                  ...
                  >
                    ...
                    <div className="mt-1 flex flex-wrap gap-2 text-xs text-muted-foreground">
                      ...
                      {endpoint.has_api_key && (
                        <span>{endpoint.api_key_preview ?? t.settings.customEndpoints.apiKeySet}</span>
                      )}
                    </div>
                  </button>
                  <div className="flex items-center gap-2 sm:justify-end">
                    ...
                    {endpoint.source !== 'direct-config' && (
                      <Button
                        ...
                        size="icon-sm"
                        title={t.settings.customEndpoints.delete}
                        variant="ghost"
                      >
          ...
          <div className="grid gap-3 rounded-md border border-border/50 p-3">
            ...
            <label className="grid gap-1.5 text-xs text-muted-foreground">
              ...
              <Input
                ...
                placeholder={form.id ? t.settings.customEndpoints.keepCurrentKey : t.settings.customEndpoints.optional}
                type="password"
                value={form.apiKey}
              />
            </label>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {testing ? t.settings.customEndpoints.testing : t.settings.customEndpoints.test}
              </Button>
              <Button disabled={saving || !canSave} onClick={() => void handleSave()}>
                {saving ? <Loader2 className="animate-spin" /> : <Save />}
                {saving ? t.settings.customEndpoints.saving : t.settings.customEndpoints.save}
              </Button>
              <Button
